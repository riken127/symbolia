...
const JumpIfIdentifier opcode.OpCode = 8
...
func (o OpJumpIf) OpCode() opcode.OpCode { return JumpIfIdentifier }
...
func (o OpJumpIf) Exec(ctx *types.ExecutionContext, instr types.Instruction) (int, error) {
	...
	condBool, ok := cond.(types.SymboliaBool)
	if !ok {
	...
	if condBool {
		...
		target, ok := instr.Args[0].(types.SymboliaInteger)
		if !ok {
...
func (o OpJumpIf) Name() types.SymboliaString { return "jumpif" }
